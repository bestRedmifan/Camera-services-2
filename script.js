...
((screenAngle%360)+360)%360;

if (normalizedScreenAngle===0) {

angle=Number(e.gamma)||0;

} else if (normalizedScreenAngle===90) {

angle=Number(e.beta)||0;
...
rotateLockButton.classList.add(
"active"
);
...
rotateLockButton.classList.remove(
"active"
);
...
horizonAngle+=
normalizeAngle(
targetHorizonAngle-horizonAngle
)*.14;

trackingX+=
(targetTrackingX-trackingX)*.12;

trackingY+=
(targetTrackingY-trackingY)*.12;
...
"Look Eyes: unavailable";
}

notify(
"Look Eyes could not start."
);
...
Math.min(
w-sw,
(w/2)-(sw/2)-(trackingX*(w/Math.max(w,1)))
)
);
...
Math.min(
h-sh,
(h/2)-(sh/2)-(trackingY*(h/Math.max(h,1)))
)
);
...
if (
!document.hidden &&
cameraPage?.classList.contains("active")
) {
startCamera();
}
}
